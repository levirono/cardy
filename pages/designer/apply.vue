<template>
  <section
    class="min-h-screen flex items-center justify-center bg-gradient-to-br from-rose-50 via-indigo-50 to-emerald-50 dark:bg-gradient-to-br dark:from-gray-900 dark:via-gray-800 p-4">
    <UCard class="w-full max-w-2xl shadow-xl">
      ...
      <div v-if="!designerEnabled" class="space-y-4">
        <UAlert color="warning" variant="soft" title="Applications are currently closed"
      ...
      <form v-else @submit.prevent="handleSubmit" class="space-y-6">
        <div>
          ...
          <UTextarea v-model="motivation" :rows="5"
            placeholder="Share your experience, tools you use, and what kind of designs you love creating..." required
            :disabled="loading" />
        </div>

        <div>
          <label class="block text-sm font-medium text-text mb-1">Portfolio link (optional)</label>
          <UInput v-model="portfolioUrl" type="url"
            placeholder="https://behance.net/your-work or https://dribbble.com/you" :disabled="loading" />
        </div>

        <UAlert color="info" variant="soft" title="What happens next?"
        ...
        <div class="flex items-center justify-end gap-3 pt-2">
          <UButton type="submit" color="primary" size="lg" :loading="loading" icon="i-heroicons-paper-airplane">
            Submit application
          </UButton>
        </div>
      </form>
