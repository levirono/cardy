<template>
  <section
    class="relative min-h-screen bg-gradient-to-br from-rose-50 via-indigo-50 to-emerald-50 dark:bg-gradient-to-br dark:from-gray-900 dark:via-gray-800 overflow-hidden lg:overflow-visible">

    <!-- Desktop Layout Container -->
    <div class="lg:grid lg:grid-cols-2 lg:gap-6 lg:p-8 lg:min-h-screen">

      <!-- Preview Section (Background on Mobile, Right Column on Desktop) -->
      <div class="fixed inset-0 z-0 lg:static lg:z-auto lg:h-auto overflow-hidden lg:order-last">
        <!-- Mobile Preview Container (Full screen centered) -->
        <div class="w-full h-full flex flex-col items-center justify-center p-4 lg:p-0">
          <div class="relative w-full max-w-md lg:max-w-none space-y-4">
            <CardPreview v-if="form.title || form.message || form.recipientName" :card="previewCard"
              class="rounded-2xl shadow-lg bg-bg-secondary/80 border border-border backdrop-blur-sm">
              <template #actions>
                <UButton color="neutral" variant="ghost" :to="previewPath" icon="i-heroicons-eye"
                  label="Preview page" />
              </template>
            </CardPreview>

            <div
              class="hidden lg:flex rounded-2xl shadow-lg p-4 bg-bg-secondary/80 border border-border flex-col items-center justify-center min-h-[200px]"
      ...
      <!-- Form Section (Slide-over on Mobile, Left Column on Desktop) -->
      <div
        class="fixed inset-0 z-10 lg:static lg:z-auto transform transition-transform duration-300 ease-out bg-gradient-to-br from-rose-50/95 via-indigo-50/95 to-emerald-50/95 dark:from-gray-900/95 dark:via-gray-800/95 lg:bg-none lg:bg-transparent overflow-y-auto lg:overflow-visible"
        :class="[showForm ? 'translate-x-0' : '-translate-x-full', 'lg:translate-x-0']" @touchstart="handleTouchStart"
        @touchmove="handleTouchMove" @touchend="handleTouchEnd">
        <div class="p-4 pt-16 lg:p-0"> <!-- Added top padding for mobile toggle area -->
          ...
          before:blur-3xl before:animate-liquid-glass">
            <template #header>
              <div class="flex items-center justify-between">
                ...
                <div class="lg:hidden text-xs text-text/50 flex items-center gap-1">
                  <UIcon name="i-heroicons-arrows-right-left" />
                  <span>Swipe left to preview</span>
                </div>
              </div>
            </template>
            ...
            <form class="grid gap-6" @submit.prevent="onSubmit">
              ...
              <UFormGroup label="Message" :ui="{ label: 'text-text' }">
                <!-- Text Toolbar -->
                <div
                  class="flex flex-wrap items-center gap-2 mb-2 p-1.5 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-border">
                  <!-- Font Family -->
                  <USelectMenu v-model="form.fontFamily" :options="(fontOptions as any)" option-attribute="label"
                    value-attribute="value" class="min-w-[140px]" :uiMenu="{ height: 'h-60' }">
                    <template #default="{ open }">
                      <UButton color="neutral" class="w-full justify-between">
                        <span class="truncate" :style="fontStyle">{{fontOptions.find(f => f.value ===
                          form.fontFamily)?.label || 'Font' }}</span>
                  ...
                  <!-- Font Size -->
                  <USelect v-model="form.messageSize" :options="messageSizeOptions" option-attribute="label"
                    value-attribute="value" class="w-28" />

                  <div class="w-px h-5 bg-gray-300 dark:bg-gray-700 mx-1"></div>

                  <!-- Alignment -->
                  <div
                    class="flex items-center bg-white dark:bg-gray-800 rounded-md border border-gray-200 dark:border-gray-700 p-0.5 shadow-sm">
                    <UButton icon="i-heroicons-bars-3-bottom-left" variant="ghost"
                      :color="form.messageAlign === 'left' ? 'primary' : 'neutral'" size="xs"
                      @click="form.messageAlign = 'left'" />
                    <UButton icon="i-heroicons-bars-3" variant="ghost"
                      :color="form.messageAlign === 'center' ? 'primary' : 'neutral'" size="xs"
                      @click="form.messageAlign = 'center'" />
                    <UButton icon="i-heroicons-bars-3-bottom-right" variant="ghost"
                      :color="form.messageAlign === 'right' ? 'primary' : 'neutral'" size="xs"
                      @click="form.messageAlign = 'right'" />
                  </div>
                </div>

                <UTextarea v-model="form.message" :rows="10" placeholder="Write something lovely..."
                  class="w-full min-w-0 p-3 bg-blue-100/30 dark:bg-blue-900/40 rounded shadow focus:ring-2 focus:ring-primary/50 transition-all duration-200 text-text" />
              </UFormGroup>
              ...
              <div class="grid gap-4 sm:grid-cols-3 bg-bg-secondary/50 p-4 rounded-xl border border-border">
                <UFormGroup label="Media type" :ui="{ label: 'text-text' }">
                  <USelect v-model="form.mediaType" :options="mediaOptions" option-attribute="label"
              ...
              <div class="grid gap-4 sm:grid-cols-3 bg-bg-secondary/50 p-4 rounded-xl border border-border">
                ...
                <UFormGroup label="Title alignment" :ui="{ label: 'text-text' }">
                  <USelect v-model="form.titleAlign" :options="alignOptions" option-attribute="label"
                    value-attribute="value"
                    class="p-3 bg-blue-100/30 dark:bg-blue-900/40 rounded shadow focus:ring-2 focus:ring-primary/50 transition-all duration-200 text-text" />
                </UFormGroup>
              </div>
              <div class="grid gap-4 sm:grid-cols-3 bg-bg-secondary/50 p-4 rounded-xl border border-border">
                <UFormGroup label="Title size" :ui="{ label: 'text-text' }">
                  <USelect v-model="form.titleSize" :options="titleSizeOptions" option-attribute="label"
                    value-attribute="value"
                    class="p-3 bg-blue-100/30 dark:bg-blue-900/40 rounded shadow focus:ring-2 focus:ring-primary/50 transition-all duration-200 text-text" />
                </UFormGroup>
              </div>
          ...
          <!-- Inline Mobile Preview (Scroll down to see) -->
          <div class="mt-8 space-y-6 lg:hidden mb-24">
            <div class="flex items-center gap-2 text-text/60 px-1">
              <UIcon name="i-heroicons-eye" class="w-5 h-5" />
              <h3 class="font-medium text-sm border-b border-border/50 pb-1 flex-1">Live Preview</h3>
            </div>
            ...
            <div
              class="flex rounded-2xl shadow-lg p-4 bg-bg-secondary/80 border border-border flex-col items-center justify-center min-h-[200px]"
              :style="{ background: form.backgroundColor, transition: 'background 0.3s' }">
              <div class="w-full h-full flex items-center justify-center">
                <MediaRenderer :media-type="form.mediaType as any" :media-url="form.mediaUrl as any"
                  :background-color="form.backgroundColor as any" class="w-full h-full object-contain" />
      ...
      <button @click="showForm = !showForm"
        class="lg:hidden fixed bottom-6 right-6 z-50 bg-primary/90 backdrop-blur-md text-white rounded-full p-4 shadow-2xl hover:scale-110 transition-all duration-300 flex items-center justify-center border-2 border-white/20">
        <UIcon :name="showForm ? 'i-heroicons-arrow-left' : 'i-heroicons-pencil-square'" class="w-6 h-6" />
      </button>
    </div>
...
// Touch event handling for sliding form away
let touchStartX = 0
let touchStartY = 0
...
// We just track the start and end to determine if it's a swipe used to close the form
// For dragging visual feedback, it complicates things with the scroll, so we'll stick to a simple swipe-to-close gesture
function handleTouchMove(e: TouchEvent) {
  // Can add logic here if needed, but browser scroll handling is tricky
}
...
function handleTouchEnd(e: TouchEvent) {
  ...
  // If dragged left (negative deltaX) significantly and horizontally
  if (deltaX < -minSwipeDistance && Math.abs(deltaY) < Math.abs(deltaX)) {
    // Swipe left -> Hide form (Show preview)
    showForm.value = false
  }

  // Note: Bringing the form back is done via the button, as the form is hidden
}



const fontStyle = computed(() => {
  if (!form.fontFamily) return undefined
  const option = fontOptions.find(f => f.value === form.fontFamily)
  return option ? { fontFamily: option.fontStack } : { fontFamily: form.fontFamily }
})

const getFontOptionStyle = (value: string) => {
  const option = fontOptions.find(f => f.value === value)
  return option ? { fontFamily: option.fontStack } : undefined
}
...
const previewCard = computed<PreviewCard>(() => ({
  ...
  fontFamily: fontOptions.find(f => f.value === form.fontFamily)?.fontStack || form.fontFamily,
...
const fontOptions: FontOption[] = [
  { label: 'Inter (Clean Sans)', value: 'inter', fontStack: "Inter, ui-sans-serif, system-ui, -apple-system, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, 'Noto Sans', 'Apple Color Emoji', 'Segoe UI Emoji'" },
  { label: 'Poppins (Modern Sans)', value: 'poppins', fontStack: "'Poppins', ui-sans-serif, system-ui, -apple-system, 'Segoe UI', Roboto, 'Helvetica Neue', Arial" },
  { label: 'Playfair Display (Serif)', value: 'playfair', fontStack: "'Playfair Display', ui-serif, Georgia, Cambria, 'Times New Roman', Times, serif" },
  { label: 'Lobster (Decorative)', value: 'lobster', fontStack: "'Lobster', 'Segoe Script', 'Comic Sans MS', cursive, sans-serif" },
  { label: 'Dancing Script (Handwritten)', value: 'dancing', fontStack: "'Dancing Script', 'Segoe Script', 'Comic Sans MS', cursive, sans-serif" }
]
const alignOptions = [
